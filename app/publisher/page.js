...
export default function PublisherPage(){
 ...
 return <main style={{maxWidth:1150,margin:'40px auto',padding:'0 20px',fontFamily:'Arial,sans-serif'}}>
  ...
  <p>Publisher — menerbitkan document_markdown menjadi Google Docs ke folder output yang ditentukan.</p>
  <p style={{fontSize:13,color:'#666'}}>Publisher tidak menggunakan Gemini. Google Docs memerlukan GOOGLE_SERVICE_ACCOUNT_JSON; folder root opsional melalui GOOGLE_DRIVE_ROOT_FOLDER_ID.</p>
  <div style={{display:'flex',gap:18,alignItems:'center',margin:'20px 0',flexWrap:'wrap'}}>
   ...
   <button onClick={run} disabled={loading} style={{padding:'10px 16px',cursor:'pointer'}}>{loading?'Menerbitkan...':skip?'Lewati Publisher':'Jalankan Publisher'}</button>
  </div>
