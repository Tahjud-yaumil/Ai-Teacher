...
function authorized(request) { const secret = env('CRON_SECRET'); if (!secret) return true; return request.headers.get('authorization') === `Bearer ${secret}`; }
...
async function saveRun(sql, { date, status, tasks = [], documents = [], publisher = null, reason = null }) { await ensureRunTable(sql); await sql`INSERT INTO pipeline_runs (tanggal,status,tasks,documents,publisher,reason,updated_at) VALUES (${date},${status},${JSON.stringify(tasks)}::jsonb,${JSON.stringify(documents)}::jsonb,${publisher ? JSON.stringify(publisher) : null}::jsonb,${reason},NOW()) ON CONFLICT (tanggal) DO UPDATE SET status=EXCLUDED.status,tasks=EXCLUDED.tasks,documents=EXCLUDED.documents,publisher=EXCLUDED.publisher,reason=EXCLUDED.reason,updated_at=NOW()`; }
async function call(origin, path, body) { const secret=env('CRON_SECRET'); const headers={'content-type':'application/json','cache-control':'no-cache'}; if(secret) headers.authorization=`Bearer ${secret}`; const response=await fetch(`${origin}${path}?cron=${Date.now()}`,{method:'POST',headers,body:JSON.stringify(body),cache:'no-store'}); const data=await response.json().catch(()=>({})); if(!response.ok) throw new Error(`${path} HTTP ${response.status}: ${data?.reason || 'request gagal'}`); return data; }
...
export async function GET(request) {
  ...
  try {
    ...
    await saveRun(sql,{date,status:'running',tasks:steps.scheduler.tasks||[]});
    steps.progress=await call(origin,'/api/progress',{tanggal:date});
    if(steps.progress?.status!=='success' && steps.progress?.status!=='no_tasks') throw new Error(`Progress Manager: ${steps.progress?.reason || 'gagal'}`);
    steps.content_generator=await call(origin,'/api/content-generator-v4',{tanggal:date,subab:'subab3'});
    if(steps.content_generator?.status!=='success') throw new Error(`Content Generator: ${steps.content_generator?.reason || 'gagal'}`);
    const documents=(steps.content_generator.documents||[]).filter(doc=>doc?.status==='success');
    steps.publisher=documents.length?await call(origin,'/api/publisher',{tanggal:date,documents,tasks:steps.scheduler.tasks||[]}):{agent:'publisher',status:'skipped',reason:'Tidak ada dokumen sukses untuk diterbitkan.'};
    await saveRun(sql,{date,status:steps.publisher?.status==='success'?'success':'error',tasks:steps.scheduler.tasks||[],documents,publisher:steps.publisher,reason:steps.publisher?.status==='success'?null:(steps.publisher?.reason||'Publisher gagal.')});
    return NextResponse.json({agent:'daily_pipeline',status:steps.publisher?.status==='success'?'success':'error',tanggal:date,timezone:'Asia/Jakarta',ai_used:true,quality_review:'skipped',steps,summary:{tasks:(steps.scheduler.tasks||[]).length,documents_generated:documents.length,documents_sent:steps.publisher?.documents_sent||0},duration_ms:Date.now()-startedAt});
  } catch(error) {
