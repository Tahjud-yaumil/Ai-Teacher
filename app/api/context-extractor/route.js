...
async function resolveBlob(sql,book,mapel,kelas){
  ...
  if(book.blob_path){try{const d=await issueSignedToken({pathname:book.blob_path,operations:['get'],validUntil:Date.now()+300000,storeId:e.BLOB_STORE_ID});const s=await presignUrl(d,{operation:'get',pathname:book.blob_path,access:'private',validUntil:Date.now()+120000,useCache:false});if((await fetch(s.presignedUrl,{method:'HEAD',cache:'no-store'})).ok)return{pathname:book.blob_path,source:'database'};}catch{}}
  const inv=await list({prefix,limit:1000});const base=String(book.file_name||'').replace(/\.[^.]+$/,'').toLowerCase();const m=inv.blobs.filter(b=>String(b.pathname).toLowerCase().includes(base)).sort((a,b)=>new Date(b.uploadedAt)-new Date(a.uploadedAt))[0];
...
async function process(sql,task){
  ...
  const kelas=task.jenis_kegiatan==='Ekstrakurikuler'?'-':task.kelas;const rows=await sql`SELECT * FROM books WHERE aktif=TRUE AND mapel=${task.mapel} AND kelas=${kelas} ORDER BY created_at DESC LIMIT 1`;if(!rows.length)return{task_id:task.task_id,status:'book_missing',reason:`Tidak ada buku untuk ${task.mapel} ${kelas}.`};
  const book=rows[0];const blob=await resolveBlob(sql,book,task.mapel,kelas);const {pages,totalPages}=await loadPages(blob.pathname);const progress=(await sql`SELECT * FROM progress WHERE sekolah=${task.sekolah} AND mapel=${task.mapel} AND kelas=${kelas} LIMIT 1`)[0]||null;const anchor=Number(progress?.halaman_akhir||0);
  const toc=findToc(pages);const unit=(tocUnits(pages,toc)[0])||null;if(!unit)throw new Error(`Daftar Isi tidak menemukan unit pertama untuk ${task.mapel} ${kelas}.`);const actual=findUnitPage(pages,toc,unit);if(!actual)throw new Error(`Awal materi ${unit.type==='tema'?'Tema':'Bab'} I tidak ditemukan.`);
  const sub=findSubPage(pages,actual.page)||{page:actual.page,label:'A',title:'Subbab berikutnya'};const start=Math.max(1,anchor?anchor+1:sub.page);const endRaw=nextSubPage(pages,start,sub.label);const end=Math.min(totalPages,endRaw>=0?endRaw:Math.min(totalPages,start+8));const pageIndexes=[];for(let i=start-1;i<end;i++)pageIndexes.push(i);
  const selected=pageIndexes.map(i=>({page:i+1,text:cleanPage(pages[i],unit.title)})).filter(x=>x.text.length>40);const context=selected.map(x=>`[Halaman ${x.page}] ${x.text}`).join('\n\n').slice(0,18000);const unitName=`${unit.type==='tema'?'Tema':'Bab'} ${unit.roman} ${unit.title}`;const contamination=selected.filter(x=>{const t=norm(x.text);const unitMarkers=[...t.matchAll(/\b(?:bab|tema)\s+([ivx]+|\d+)\b/gi)].map(m=>m[0]);return unitMarkers.some(m=>sim(m,unitName)<0.25);}).length;
